...
export async function onRequestGet({ request, env }) {
  try {
    ...
    const [stats, families, vehicleProducts, rewards, chart, topProducts, topVehicles, provinces, ages, invoices] = await Promise.all([
      env.DB.prepare("SELECT (SELECT COUNT(*) FROM invoices WHERE datetime(issued_at)>=datetime(?1) AND datetime(issued_at)<datetime(?2)) invoices,(SELECT COALESCE(SUM(amount_cents),0) FROM invoices WHERE datetime(issued_at)>=datetime(?1) AND datetime(issued_at)<datetime(?2)) salesCents,(SELECT COUNT(*) FROM users WHERE role='customer' AND status='active') customers,(SELECT COUNT(*) FROM rewards WHERE active=1) rewardItems,(SELECT COUNT(*) FROM redemptions WHERE status='requested') pendingRedemptions,(SELECT COUNT(*) FROM installations i JOIN vehicles v ON v.id=i.vehicle_id WHERE i.status!='void' AND ((i.next_service_at IS NOT NULL AND datetime(i.next_service_at)<=CURRENT_TIMESTAMP) OR (i.next_service_km IS NOT NULL AND v.odometer_km>=i.next_service_km))) expiredWarranties,(SELECT COUNT(*) FROM warranty_events WHERE event_type='extended' AND datetime(created_at)>=datetime(?1) AND datetime(created_at)<datetime(?2)) extendedWarranties").bind(startAt,endAt).first(),
