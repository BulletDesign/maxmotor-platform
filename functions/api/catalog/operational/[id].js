...
export async function onRequestPatch({ request, env, params }) {
  try {
    ...
    const current = await env.DB.prepare("SELECT family_id familyId,name,warranty_days warrantyDays,warranty_km warrantyKm,active FROM operational_products WHERE id=?1").bind(params.id).first();
    ...
    await env.DB.batch([
      env.DB.prepare("UPDATE operational_products SET family_id=?1,name=?2,warranty_days=?3,warranty_km=?4,active=?5 WHERE id=?6").bind(familyId, name, warrantyDays, warrantyKm, active, params.id),
      env.DB.prepare("INSERT INTO audit_log(id,actor_user_id,action,entity_type,entity_id,metadata_json) VALUES(?1,?2,'product.update','operational_product',?3,?4)").bind(crypto.randomUUID(), actor.id, params.id, JSON.stringify({ familyId, name, warrantyDays, warrantyKm, active })),
    ]);
    return json({ ok: true });
