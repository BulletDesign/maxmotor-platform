...
async function createFriendlyCustomerCode(db) {
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (originProvince.length < 3) throw new HttpError(400, "Provincia de origen requerida");
    ...
    const id = crypto.randomUUID(); const customerCode = await createFriendlyCustomerCode(env.DB); const secured = await hashPassword(password);
