...
export async function onRequestPatch({request,env,params}){
  try{assertSameOrigin(request);const actor=await requireUser(request,env.DB,["employee","superadmin"]);const body=await readJson(request);const fullName=String(body.fullName||"").trim();const phone=String(body.phone||"").trim();const email=String(body.email||"").trim().toLowerCase();if(fullName.length<3||!email)throw new HttpError(400,"Datos incompletos");await env.DB.batch([env.DB.prepare("UPDATE users SET full_name=?1,phone=?2,email=?3,updated_at=CURRENT_TIMESTAMP WHERE id=?4 AND role='customer'").bind(fullName,phone,email,params.id),env.DB.prepare("INSERT INTO audit_log(id,actor_user_id,action,entity_type,entity_id,metadata_json) VALUES(?1,?2,'customer.update','user',?3,?4)").bind(crypto.randomUUID(),actor.id,params.id,JSON.stringify({fullName,email,phone}))]);return json({ok:true});}catch(error){return handleError(error);}
}
