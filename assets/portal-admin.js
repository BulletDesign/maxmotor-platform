...
async function openCustomerFile(id) {
  ...
  const invoiceForm = document.querySelector("#customer-invoice-form"); invoiceForm.elements.customerCode.value = customer.customerCode; invoiceForm.elements.issuedAt.value = new Date().toISOString().slice(0, 10);
...
document.querySelector("#customer-invoice-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; const values = Object.fromEntries(new FormData(form)); try { const data = await api("/api/admin/points/award", { method: "POST", body: JSON.stringify({ customerCode: values.customerCode, invoiceNumber: values.invoiceNumber, amountCents: Math.round(Number(values.amount) * 100), issuedAt: new Date(`${values.issuedAt}T12:00:00`).toISOString() }) }); setMessage(fileMessage, `${Number(data.points).toLocaleString("es-EC")} Traction Points acreditados.`, true); form.elements.invoiceNumber.value = ""; form.elements.amount.value = ""; await Promise.all([openCustomerFile(activeCustomerId), loadOverview()]); } catch (error) { setMessage(fileMessage, error.message); } });
...
document.querySelector("#award-points-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; const values = Object.fromEntries(new FormData(form)); report("#points-message", ""); try { const data = await api("/api/admin/points/award", { method: "POST", body: JSON.stringify({ customerCode: values.customerCode, invoiceNumber: values.invoiceNumber, amountCents: Math.round(Number(values.amount) * 100), issuedAt: new Date(`${values.issuedAt}T12:00:00`).toISOString() }) }); report("#points-message", `Factura registrada: ${Number(data.points).toLocaleString("es-EC")} Traction Points acreditados.`, true); form.reset(); await Promise.all([loadOverview(), loadCustomers("")]); } catch (error) { report("#points-message", error.message); } });
