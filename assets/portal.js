...
async function loadDashboard(user) {
  ...
  const [vehiclesData, pointsData, rewardsData, redemptionsData] = await Promise.all([api("/api/vehicles"), api("/api/points/summary"), api("/api/rewards"), api("/api/redemptions?role=customer")]);
...
document.querySelector("#rewards-grid").addEventListener("click", async (event) => { const button = event.target.closest("[data-reward-id]"); if (!button) return; const result = document.querySelector("#portal-message"); try { await api("/api/redemptions", { method: "POST", body: JSON.stringify({ rewardId: button.dataset.rewardId }) }); result.textContent = "Solicitud enviada. Maxmotor confirmara tu canje."; result.classList.add("is-success"); result.hidden = false; button.disabled = true; const data=await api("/api/redemptions?role=customer"); renderRedemptions(data.redemptions||[]); } catch (error) { result.textContent = error.message; result.hidden = false; } });
