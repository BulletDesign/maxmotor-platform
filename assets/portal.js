...
function renderMovements(movements) { document.querySelector("#points-movements").innerHTML = movements.length ? movements.map((item) => `<article class="data-row"><strong>${escapeHtml(item.description)}${item.invoiceNumber ? `<small>Factura #${escapeHtml(item.invoiceNumber)}</small>` : ""}</strong><span>${new Date(item.created_at).toLocaleDateString("es-EC")}</span><em>${item.points > 0 ? "+" : ""}${Number(item.points).toLocaleString("es-EC")} TP</em></article>`).join("") : '<p class="empty-state">Tu primer movimiento aparecera cuando registremos una factura.</p>'; }
function renderInvoiceHistory(invoices) {
  const container = document.querySelector("#invoice-history");
  container.innerHTML = invoices.length ? invoices.map((invoice) => {
    ...
    const points = invoice.pointsEnabled ? `+${Number(invoice.pointsEarned).toLocaleString("es-EC")} TP acreditados` : "Factura sin acreditacion de TP";
    return `<details class="invoice-card"><summary><div><span>FACTURA</span><strong>#${escapeHtml(invoice.invoiceNumber)}</strong></div><div><span>${new Date(invoice.issuedAt).toLocaleDateString("es-EC")}</span><strong>USD ${Number(invoice.amountCents / 100).toFixed(2)}</strong></div><em class="${invoice.pointsEnabled ? "is-earned" : ""}">${points}</em></summary><div class="invoice-card__detail"><ul>${items}</ul><small>Registrada por ${escapeHtml(invoice.createdByName)}</small></div></details>`;
  }).join("") : '<p class="empty-state">Aun no existen facturas registradas en tu cuenta.</p>';
}
...
function renderRequests(redemptions, coupons) {
  ...
  const couponRows = coupons.map((item) => `<article class="data-row coupon-request"><strong>10% OFF / ${escapeHtml(item.code)}</strong><span>${escapeHtml(item.terms)}${item.expiresAt ? `<br>Vigente hasta ${new Date(item.expiresAt).toLocaleDateString("es-EC")}` : ""}</span><div class="coupon-state"><em>${couponStatus[item.status] || escapeHtml(item.status)}</em>${item.status === "available" ? `<button type="button" data-coupon-request="${item.id}">Solicitar uso</button>` : ""}</div></article>`);
...
async function loadDashboard(user) {
  ...
  renderInvoiceHistory(pointsData.invoices || []);
...
document.querySelector("#register-form").addEventListener("submit", async (event) => { event.preventDefault(); const form = event.currentTarget; const values = Object.fromEntries(new FormData(form)); const payload = { fullName: values.fullName, phone: values.phone, birthDate: values.birthDate, originProvince: values.originProvince, email: values.email, password: values.password, consent: form.elements.consent.checked, welcomeOffer, vehicle: { brand: values.brand, model: values.model, modelYear: values.modelYear, plate: values.plate } }; setBusy(form, true); try { const result = await api("/api/auth/register", { method: "POST", body: JSON.stringify(payload) }); history.replaceState({}, "", "/MiMaxmotor"); await loadDashboard(result.user); if (result.welcomePoints > 0 || result.welcomeCoupon) { selectView("points"); const couponText = result.welcomeCoupon ? ` y tu cupon ${result.welcomeCoupon}` : ""; report("#portal-message", `${Number(result.welcomePoints || 0)} TP de bienvenida acreditados${couponText}.`, true); } } catch (error) { showMessage(error.message); } finally { setBusy(form, false); } });
...
document.querySelector("#redemptions-list").addEventListener("click", async (event) => { const button = event.target.closest("[data-coupon-request]"); if (!button) return; button.disabled = true; try { await api("/api/coupons", { method: "POST", body: JSON.stringify({ couponId: button.dataset.couponRequest }) }); report("#portal-message", "Cupon solicitado. Un asesor validara el 10% OFF y veras aqui cuando sea aceptado.", true); const [redemptions, coupons] = await Promise.all([api("/api/redemptions?role=customer"), api("/api/coupons")]); renderRequests(redemptions.redemptions || [], coupons.coupons || []); } catch (error) { report("#portal-message", error.message); button.disabled = false; } });
