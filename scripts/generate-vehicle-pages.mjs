...
const projectRoutes = {
  towing: "/fichas/tiro-hd",
  frontProtection: "/fichas/bumpers-bullbars-guardachoques",
  racks: "/fichas/parrillas-carga",
  portabike: "/fichas/portabicicletas",
  underbody: "/ingenieria",
  polyurethane: "/maxlining/vehiculos",
  rollbars: "/fichas/rollbar-zr",
  lighting: "/fichas/luces-led",
  suspension: "/tough-dog",
  covers: "/fichas/tapas-balde-camionetas",
};

const projectLabels = {
  towing: "Barra de tiro",
  frontProtection: "Protección frontal",
  racks: "Sistema de carga",
  portabike: "Portabicicletas",
  underbody: "Protección inferior",
  polyurethane: "Maxlining",
  rollbars: "Rollbar",
  lighting: "Iluminación",
  suspension: "Suspensión",
  covers: "Tapa de balde",
};
...
function pageFor(pickup) {
  ...
  const projectQuote = (item) => `https://wa.me/593960855932?text=${encodeURIComponent(`Hola Maxmotor 4x4, vi el proyecto ${item.title} para ${name}. Quiero cotizarlo para año / cabina / versión:`)}`;
  return `<!doctype html>
<html lang="es-EC">
<head>
  <meta charset="utf-8">
  ...
  <link rel="stylesheet" href="/assets/seo-product.css?v=20260825-2">
...
<body>
  ...
  <main class="dmax-page vehicle-page">
    ...
    <section class="dmax-options" id="accesorios" aria-labelledby="accessory-title">
      <div class="dmax-section-title"><div><p class="dmax-kicker">01 / HOT SELLERS</p><h2 id="accessory-title">Herramientas.<br>No juguetes.</h2></div><p>${escapeHtml(aliasText)} No publicamos compatibilidades genéricas: primero entendemos el uso y después confirmamos el componente.</p></div>
    ...
    <section class="vehicle-projects" aria-labelledby="vehicle-projects-title"><div class="dmax-section-title"><div><p class="dmax-kicker">PROYECTOS REALES / MAXMOTOR</p><h2 id="vehicle-projects-title">Hecho sobre<br>${escapeHtml(pickup.model)}.</h2></div><p>Trabajos reales identificados para este modelo. Abre cada tarjeta, amplía los detalles y cotiza esa configuración.</p></div><div class="vehicle-projects__grid">${realProjects.map((item) => `<a class="vehicle-project-card" href="${projectRoutes[item.family] || "/productos/"}" data-media-lightbox data-media-src="${item.src}" data-media-title="${escapeHtml(`${item.title} / ${name}`)}" data-media-quote="${projectQuote(item)}" aria-label="Ampliar ${escapeHtml(item.title)} para ${escapeHtml(pickup.model)}"><span class="vehicle-project-card__media"><img src="${item.src}" alt="${escapeHtml(item.alt)}" width="1200" height="900" loading="lazy" decoding="async"></span><span class="vehicle-project-card__copy"><small>${escapeHtml(projectLabels[item.family] || "Proyecto Maxmotor")}</small><strong>${escapeHtml(item.title)}</strong><b>Ampliar y cotizar <i>↗</i></b></span></a>`).join("")}</div></section>` : ""}
