...
<body>
  ...
  <main class="detail-shell">
    ...
    <section class="product-body">
      ...
      <section class="project-gallery" aria-labelledby="gallery-title"><div class="section-head"><div><span class="eyebrow">02 / Proyectos reales</span><h2 id="gallery-title">Instalado por Maxmotor.</h2></div><p>Referencias visuales de trabajos reales. Abre una tarjeta para ampliar la imagen y cotizar el producto observado.</p></div><div class="project-gallery__grid">${gallery.map((item, index) => `<a class="project-gallery__card" href="${quote}" target="_blank" rel="noopener" data-media-lightbox data-media-src="${item.src}" data-media-title="${escapeHtml(item.title)}" data-media-quote="${quote}" aria-label="Ampliar ${escapeHtml(item.title)} y consultar ${escapeHtml(product.name)}"><span class="project-gallery__media"><img src="${item.src}" alt="${escapeHtml(item.alt)}" width="1200" height="900" loading="${index === 0 ? "eager" : "lazy"}" decoding="async"></span><span class="project-gallery__copy"><small>Proyecto real / Maxmotor 4x4</small><strong>${escapeHtml(item.title)}</strong><b>Ampliar y cotizar <i>↗</i></b></span></a>`).join("")}</div></section>` : ""}
